package processing.test.society;
...
public class society extends PApplet {
...
class Engine {
  ...
  float up = height * 0.45f;
  float down = -height * 0.45f;
  float left = width * 0.60f;
  float right = -width * 0.60f;
  ...
  public void display() {
    ...
    if (withChild) {
      ...
      for (int i=0; i < childIndexes.size(); i++) {
        ...
        int line_color = _color; //(childIndexes.size() > 1) ? _color : color(255);
